...
!(async() => {
    ...
    if (!token) {
        message = "❌未找到签到Token，请先打开华住会的新签到页面获取\n";
        await notify();
        return;
    }

    let signStatus = await status();
    if (signStatus?.signToday === true) {
        message += "签到:今日已签到\n";
    } else {
        await signin();
        signStatus = await status();
    }
    if (signStatus?.memberPoint != null) {
        message += `当前积分:${signStatus.memberPoint}`;
    }
    await notify();
})()
.catch((e) => {
...
function getToken() {
    if ($request && $request.method != 'OPTIONS') {
        ...
        const headerToken = getHeader(headers, 'user-token');
        const cookieToken = getCookieValue(getHeader(headers, 'cookie'), 'userToken');
        const value = cookieToken || headerToken;
        if (value) {
            const saved = $.setdata(value, token_key);
            if (saved) {
                $.msg($.name, '', '获取新签到Token成功🎉');
            }
        }
    }
}
    ...
    return new Promise((resolve) => {
        ...
        $.get(signinRequest, (error, response, data) => {
            try {
                ...
                if (result?.code == 200 && String(result?.businessCode) === '1000') {
                    if (result?.content?.signResult === true) {
                        message += `签到:获得积分:${result.content.point ?? 0}`;
                        if (result.content.activityPoints != null) {
                            message += `，活跃值:${result.content.activityPoints}`;
                        }
                        message += '\n';
                    } else {
                        message += `签到:${result?.message || result?.responseDes || '未成功，请检查今日是否已签到'}\n`;
                    }
                } else {
                    message += `❌签到失败:${result?.message || result?.responseDes || `HTTP ${response?.statusCode || '未知'}`}\n`;
                }
            } catch (e) {
                message += `❌签到请求异常:${e.message || e}\n`;
                $.logErr(e);
            } finally {
                resolve();
            }
    ...
    return new Promise((resolve) => {
        ...
        $.get(statusRequest, (error, response, data) => {
            try {
                ...
                message += `❌状态查询失败:${result?.message || result?.responseDes || `HTTP ${response?.statusCode || '未知'}`}\n`;
            } catch (e) {
                message += `❌状态查询异常:${e.message || e}\n`;
                $.logErr(e);
            }
            resolve(null);
        });
